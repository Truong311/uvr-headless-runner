...
from lib_v5.scnet import SCNet
...
if not is_macos:
    try:
        ...
        directml_imported = True
    except ImportError:
        directml_imported = False
        torch_directml = None
...
def get_gpu_info():
    ...
    if not is_macos and directml_imported and torch_directml is not None:
    ...
    def __init__(self, model_data: ModelData, 
        ...
        if self.is_gpu_conversion >= 0:
            if mps_available:
                ...
                if directml_available and self.is_use_directml and directml_imported and torch_directml is not None:
...
class SeperateMDXC(SeperateAttributes):        
    ...
    def demix(self, mix):
        ...
        if self.is_roformer:
            ...
            elif 'band_SR' in self.roformer_config.model or 'sources' in self.roformer_config.model:
                # SCNet model - uses band_SR, band_stride, band_kernel parameters
        ...
        # SCNet uses model.hop_size, Roformer uses audio.hop_length
        if hasattr(self.roformer_config.model, 'hop_size'):
            hop_length = self.roformer_config.model.hop_size
        elif hasattr(self.roformer_config.audio, 'hop_length'):
            hop_length = self.roformer_config.audio.hop_length
        else:
            hop_length = 441  # Default fallback
